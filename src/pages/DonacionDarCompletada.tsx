...
import { Heart, Sparkles, CheckCircle2, Home, BookOpen, ArrowRight, HandHeart } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useTranslation } from 'react-i18next';

export default function DonacionDarCompletada() {
      ...
      <div className="max-w-2xl w-full mx-auto">
        ...
        >
          ...
          <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold uppercase tracking-wider mb-3">
            <HandHeart className="w-3.5 h-3.5 text-emerald-600" /> {t('donations.darBadge')}
          </span>
          <h1 className="text-3xl sm:text-4xl font-gordita font-bold text-[#2D4B73] mb-4">
            {t('donations.darCompleteTitle')}
          </h1>

          {/* Custom Gratitude Message */}
          <div className="bg-emerald-50/50 border border-emerald-200/70 rounded-2xl p-6 sm:p-8 mb-8 text-left shadow-sm">
            <p className="text-[#2D4B73]/90 text-base sm:text-lg leading-relaxed font-medium">
              {t('donations.darCompleteMessage')}
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              to="/dar"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#2D4B73] hover:bg-[#1f3552] text-white font-bold py-3.5 px-8 rounded-2xl shadow-lg transition-all text-sm cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
            >
              <span>{t('donations.viewWaysToGive')}</span>
