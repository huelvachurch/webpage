...
export default function MisDatos() {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (confirmEmail.trim().toLowerCase() !== user.email?.toLowerCase()) {
      setDeleteError('El email ingresado no coincide con tu email actual.');
      return;
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            ...
            {/* Academic Program (Alumno state) */}
            <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm text-center md:text-left">
              <h2 className="text-xl font-kenao text-primary mb-2 flex items-center gap-2 justify-center md:justify-start">
                <GraduationCap className="w-5 h-5 text-secondary" />
                Rol Académico
              </h2>
              <p className="text-xs text-primary/55 mb-6 leading-relaxed">
                Para inscribirte y ver programas en <strong>Mis Cursos</strong>, se requiere el rol de <strong>Alumno</strong> asignado por el equipo administrativo.
              </p>
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col items-center gap-4 text-center">
                ...
                <div>
                  <h4 className="font-bold text-sm text-primary">Estado de Alumno</h4>
                  <div className="mt-1.5 flex justify-center">
                    {isAlumno ? (
                      <span className="text-emerald-700 font-bold bg-emerald-50 text-[10px] px-2.5 py-1 rounded-full uppercase tracking-wider">Asignado</span>
                    ) : hasRequestedAlumno ? (
                      <span className="text-amber-700 font-bold bg-amber-50 text-[10px] px-2.5 py-1 rounded-full uppercase tracking-wider">Esperando Asignación</span>
                    ) : (
                      <span className="text-slate-500 font-bold bg-slate-200 text-[10px] px-2.5 py-1 rounded-full uppercase tracking-wider">No solicitado</span>
                    )}
                  </div>
                </div>

                {!isAlumno && !hasRequestedAlumno && (
                  <button
                    type="button"
                    onClick={() => navigate('/cursos')}
                    className="w-full py-3.5 px-4 bg-primary text-white hover:bg-primary/95 shadow-sm rounded-xl font-black text-xs uppercase tracking-wider transition-all cursor-pointer border border-transparent"
                  >
                    Solicitar ser Alumno
                  </button>
                )}

                {hasRequestedAlumno && !isAlumno && (
                  <p className="text-[11px] text-amber-800 bg-amber-50/50 p-2.5 rounded-xl border border-amber-100 leading-relaxed font-bold">
                    Se ha enviado la solicitud de Rol de Alumno. El equipo administrativo dará respuesta a tu solicitud muy pronto.
                  </p>
                )}
              </div>
            </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/45 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-10 max-w-md w-full shadow-2xl relative overflow-hidden">
            ...
            <label className="block text-xs font-black uppercase tracking-wider text-slate-500 mb-2">
              Para confirmar, escribe tu correo electrónico actual:
            </label>
            <input 
              type="text"
              required
              ...
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 focus:bg-white text-sm mb-4 outline-none transition-all"
            />
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button
                ...
                className="px-5 py-3.5 bg-red-650 hover:bg-red-700 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md cursor-pointer flex items-center gap-1.5"
              >
                {deleting ? (
                  <>
