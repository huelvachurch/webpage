...
const regex = /            \{activeTab === 'supervision' && \([\s\S]*?\n            \{\/\* TAB 1: FORMULARIO DE REUNION \*\/\}/;
                ...
                {supervisedEnrollments.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {supervisedEnrollments.map((enrollment) => {
                      ...
                      return (
                        <div key={enrollment.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden text-left p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div className="flex-grow">
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                              ...
                              {isPaused && <span className="ml-2 text-rose-500 font-bold tracking-widest uppercase text-[10px] bg-rose-50 px-2 py-0.5 rounded-full border border-rose-200">PAUSADO</span>}
                            </p>
                          </div>
                          ...
                          <div className="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                className={\`px-4 py-3 rounded-xl font-bold text-xs transition-colors shadow-sm cursor-pointer \${isPaused ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}\`}
                              >
                              ...
                              <button
                                ...
                                className="px-4 py-3 bg-rose-50 text-rose-600 hover:bg-rose-100 rounded-xl font-bold text-xs transition-colors shadow-sm cursor-pointer"
                              >
                                Detener
                              </button>
                            </div>
